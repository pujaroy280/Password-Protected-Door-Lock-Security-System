...
bool door = true;
byte rowPins[ROWS] = {1, 2, 3, 4}; //connect to the row pinouts 
of the keypad
byte colPins[COLS] = {5, 6, 7}; //connect to the column pinouts 
of the keypad
Keypad customKeypad( makeKeymap(keys), rowPins, colPins, ROWS, 
...
void ServoOpen()
{
 for (pos = 180; pos >= 0; pos -= 5) { // goes from 0 degrees 
to 180 degrees
...
void ServoClose()
{
 for (pos = 0; pos <= 180; pos += 5) { // goes from 180 degrees 
to 0 degrees
 myservo.write(pos); // tell servo to go to 
position in variable 'pos'
 delay(15); // waits 15ms for the servo 
...
to (customKey != NO_KEY)
 ...
 lcd.setCursor(data_count, 1); // move cursor to show each 
new char
